    ...
    public function getFailReason($orderId){
        ...
        if ($order->getId()){
            ...
            if (!empty($minAllowedDays)){
                ...
                if ($date->gmtTimestamp($order->getCreatedAt()) > $t){
                    $ret = $this->__("You can not create RMA for this order because minimal time period allowed for RMA after order complition is %s days", $minAllowedDays);
                }
            }
            ...
            if (!empty($maxAllowedDays)){
                ...
                if ($date->gmtTimestamp($order->getCreatedAt()) < $t){
                    $ret = $this->__("You can not create RMA for this order because maximal time period allowed for RMA after order complition is %s days", $maxAllowedDays);
                }
            }
        }
        ...
        if (!$ret){
            if (count($this->_errors) > 0){
                ...
                $ret = $this->__("You can not create RMA for this order");
            }
        }
