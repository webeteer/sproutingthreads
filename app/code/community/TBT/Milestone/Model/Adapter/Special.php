...
class TBT_Milestone_Model_Adapter_Special extends Varien_Object
{
    ...
    public function afterSaveAction($observer)
    {
        try {
            ...
            // use supplied normalized date or current normalized time
            $conditionDetails["from_date"] = $helper->getMySqlDateString($helper->getUtcTimestamp($fromDate));
            $conditionDetails["to_date"]   = empty($specialRule["to_date"])
                ? ""
                : $helper->getMySqlDateString($helper->getUtcTimestamp($specialRule["to_date"]));
    ...
    /**
     * Listens for 'rewards_special_rule_save_after' event. Right now used only for import functionality.
     * Creates the milestone rule associated.
     *
     * @param  Varien_Event_Observer $observer
     *
     * @return self
     */
    public function specialRuleSaveAfter(Varien_Event_Observer $observer)
    {
        ...
        $milestoneRule->setConditionType($specialRule->getConditionType());
        $milestoneRule->setConditionDetailsJson($specialRule->getConditionDetailsJson());
        $milestoneRule->setActionType($specialRule->getActionType());
        $milestoneRule->setActionDetailsJson($specialRule->getActionDetailsJson());
        $milestoneRule->setIsEnabled($specialRule->getIsActive());
