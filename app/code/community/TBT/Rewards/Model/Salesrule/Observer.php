...
class TBT_Rewards_Model_Salesrule_Observer extends Varien_Object
{
    ...
    public function salesruleValidatorProcess($o)
    {
        ...
        try {
            ...
            Mage::helper('rewards')->log(
                "An error occured while trying to process shopping cart points rules: " . $e->getMessage()
    ...
    public function setItemDiscountPrice($observer)
    {
        ...
        // if no STR catalog discounts, just skip (ST-2767)
        $quote = $item->getQuote();
        if (!$quote->getId() || !$quote->hasAnyAppliedCatalogRedemptions()) {
            return $this;
        }
        ...
        if (Mage::helper('tax')->discountTax($storeId)) {
            ...
            $item->setBaseDiscountCalculationPrice(
                Mage::helper('rewards/price')->getReversedCurrencyPrice(
                    $item->getRowTotalAfterRedemptionsInclTax() / $qty
                )
            );
        } else {
            $item->setDiscountCalculationPrice($item->getRowTotalAfterRedemptions() / $qty);
            $item->setBaseDiscountCalculationPrice(
                Mage::helper('rewards/price')->getReversedCurrencyPrice($item->getRowTotalAfterRedemptions() / $qty)
            );
        }
