...
class TBT_Rewards_Model_Salesrule_Actions extends Varien_Object
{
    ...
    public function getRedemptionsOptionArray()
    {
        return array(
            ...
            //self::ACTION_DEDUCT_BY_AMOUNT_SPENT => Mage::helper('rewards')->__('For every Y amount spent, deduct X points [in development]'),
    ...
    {
        return array(
            self::ACTION_DEDUCT_POINTS, self::ACTION_DEDUCT_BY_AMOUNT_SPENT, self::ACTION_DEDUCT_BY_QTY,
    ...
    {
        $isRedemption = $this->isDeductPointsAction($action) || $this->isDeductByAmountSpentAction($action)
            || $this->isDeductByQtyAction($action) || $this->isDiscountByPointsSpentAction($action);
    ...
    {
        $isDistribution = $this->isGivePointsAction($action) || $this->isGiveByQtyAction($action)
