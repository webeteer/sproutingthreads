...
class TBT_Rewards_Model_Catalogrule_Observer extends Mage_CatalogRule_Model_Observer
{
    ...
    private function checkRule($rule)
    {
        ...
        if (!(strtotime($rule->getFromDate()) <= strtotime($localDate)
            && strtotime($rule->getToDate()) >= strtotime($localDate))
        ) {
            return false;
        }
        ...
        if (!$this->isInGroup(
            ...
            explode(",", $rule->getCustomerGroupIds())
        )) {
            return false;
        }
        ...
        try {
            ...
            foreach ($items as $key => $itemInfo) {
                ...
                if (Mage::helper('rewards')->isBaseMageVersionAtLeast('1.4.0.0')
                    && (!Mage::app()->getRequest()->isAjax() || Mage::helper('rewards')->getIsAdmin())
                ) {
                    // Since the cart has changed, reset our before-redemptions value and let Sweet Tooth recalculate
                    // the discount amount.
                    // however, if it's an ajax call, it might be coming from Paypal Express review page for example
                    // so we don't want this. Check ST-2728
     ...
     * @see Mage::dispatchEvent('checkout_cart_add_product_complete', array('product'=>$product,
     ...
     */
    private function updateRedeemedPointsHash(
        $date,
        $wId,
        $gId,
        $pId,
        ...
        Mage::getSingleton('rewards/catalogrule_saver')->updateRedeemedPointsHash(
            $date, $wId, $gId, $pId, $item, $apply_rule_id, $qty, $adjustQty, $uses
        );
    ...
    public function appendPointsQuote($o)
    {
        try {
            ...
            if ($item) {
                ...
                $this->updateRedeemedPointsHash(
                    $date, $wId, $gId, $pId, $item, $apply_rule_id, $qty, true, $apply_rule_uses
                );
            ...
            Mage::getSingleton('core/session')->addError(
                Mage::helper('rewards')->__(
                    "An error occurred trying to apply the redemption while adding the product to your cart: "
                ) . $e->getMessage()
            );
        }
    }
