...
class TBT_Rewards_Manage_Customer_PointsController extends TBT_Rewards_Admin_AbstractController
{
    ...
    public function massTransferPointsAction()
    {
        try {
            ...
            foreach ($customerIds as $customer_id) {
                try {
                    ...
                    Mage::getSingleton('core/session')->addError(
                        $this->__('Failed to transfer points to customer %s', $customer->getName()) . " : "
                        . $e->getMessage()
                    );
                }
            }
            ...
            Mage::getSingleton('core/session')->addError(
                $this->__('Whoops, ran into error while trying to process your request: ') . $e->getMessage()
            );
        }
    ...
    {
        $this->loadLayout()->_setActiveMenu('rewards/customer_points')->_addBreadcrumb(
            Mage::helper('rewards')->__('Promotions'), Mage::helper('rewards')->__('Promotions')
        );
