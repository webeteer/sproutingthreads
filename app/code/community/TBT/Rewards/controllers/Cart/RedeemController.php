...
class TBT_Rewards_Cart_RedeemController extends Mage_Core_Controller_Front_Action
{
    ...
    public function cartaddAction()
    {
        ...
        try {
            ...
            foreach ($rule_list as $rule_id) {
                ...
                if (array_search(( int )$rule_id, explode(',', $quote->getCartRedemptions())) === false) {
                    ...
                    if ($customer_point_balance [$rule->getPointsCurrencyId()] < $rule->getPointsAmount()) {
                        $this->_getSess()->addError(
                            $this->__(
                                "You do have enough %s Points.", $currency_captions [$rule->getPointsCurrencyId()]
                            ) . "<br/>\n" . $this->__(
                                "The rule entitled '%s' was not applied to your cart.",
                                $rule->getStoreLabel($store) ? $rule->getStoreLabel($store) : $rule->getName()
                            )
                        );
                        $flag = false;
            ...
            $this->_getSess()->addError(
                $this->__("An error occurred while trying to apply the redemption to your cart.")
            );
        ...
        try {
            ...
            foreach ($rule_list as $rule_id) {
                ...
                if (array_search(( int )$rule_id, $applied_redemptions) === false) {
                    $this->_getSess()->addError(
                        $this->__(
                            "The rule %s was not applied to your cart.",
                            $rule->getStoreLabel($store) ? $rule->getStoreLabel($store) : $rule->getName()
                        )
                    );
                } else {
            ...
            if ($doSave) {
                ...
                $this->_getSess()->addSuccess(
                    $this->__("All requested reward redemptions were removed from your cart")
                );
            }
        } catch (Exception $e) {
            $this->_getSess()->addError(
                $this->__("An error occurred while trying to remove the redemption from your cart.")
            );
            $this->_getSess()->addError($this->__($e->getMessage()));
        }
     ...
     *
     * @param  int $item_id     The id of the quote item to load
     ...
     * params, if any
     *
     * @param  int $quote_id    The id of the quote to be loaded
